...
module.exports.run = async({ api, event, args, client, Users, Threads, global, Currencies }) => {
    ...
    if (money < 500) api.sendMessage("Bạn cần 500 đô để xem ảnh ?", event.threadID, event.messageID)
        ...
        var callback = () => api.sendMessage({ body: `Ảnh hannaUwU\nSố Ảnh: ${link.length}\n-500 đô !`, attachment: fs.createReadStream(__dirname + "/cache/1.jpg") }, event.threadID, (err, info) => setTimeout(() => api.unsendMessage(info.messageID), 20000), event.messageID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID);
