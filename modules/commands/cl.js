...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    if (!args[0]) return api.sendMessage("Bạn phải cược chẳn hoặc lẻ...", threadID, messageID);
    const choose = args[0]
    if (choose.toLowerCase() != 'chẳn' && choose.toLowerCase() != 'lẻ') return api.sendMessage("Chỉ đặt cược chẳn hoặc lẻ!", threadID, messageID)
    var money = args[1] 
if (money.toLowerCase() == "all") var money = `${moneyUser}`
    ...
    try {
        const res = (await axios.get(`https://apid.chinhle4447.repl.co/taixiu`)).data
        const image = [];
