...
const time = process.uptime(),
  ...
  const Canvas = require('canvas');
    ...
    let background = (await axios.get(encodeURI(`https://i.imgur.com/MTltYgn.png`), { responseType: "arraybuffer" })).data;
...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 1200, -270, 1700, 1800);
    ...
    ctx.fillStyle = lengthchar[id].colorBg;
    ctx.fillText(global.config.BOTNAME, 280, 435);
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/tad/SVN-AVENGEANCE.OTF`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "120px SVN-Avengeance";
    ctx.fillStyle = "#fdfdfd";
    ctx.fillText(`${z_1} : ${x_1} : ${y_1} `, 365, 560);
    ctx.restore();
    ctx.save();
