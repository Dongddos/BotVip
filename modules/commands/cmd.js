        ...
        case "loadAll": {
         ...
         //  moduleList = moduleList.map(item => item.replace(/\.js/g, ""));
		   ...
		   const { commands } = global.client;
			const { threadID, messageID } = event;
	        return api.sendMessage(`Đã load ${commands.size} modules!`, threadID, () => process.exit(1));
        }
        case "unloadAll": {
          ...
          case "count": {
		   const { commands } = global.client;
			const { threadID, messageID } = event;
	        return api.sendMessage(`Có ${commands.size} module đang chạy!`, threadID, messageID);
        }  
