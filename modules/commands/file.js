module.exports.config = {
	...
	usages: "\delfile start <text>\delfile ext <text>\delfile <text>\delfile [để trống]\delfile help\nNOTE: <text> là ký tự bạn điền vào tùy ý",
	cooldowns: 5
};
...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  api.sendMessage("Đã xóa các file sau trong thư mục modules:\n\n"+msg, event.threadID, event.messageID);
}
...
•Ví dụ: cache help`;
    ...
    if(files.length == 0) return api.sendMessage(`Không có file nào có ký tự bắt đầu bằng: ${word}`, event.threadID ,event. messageID);
  ...
  //đuôi file là..... 
  	...
  	if(files.length == 0) return api.sendMessage(`Không có file nào có ký tự kết thúc bằng: ${ext}`, event.threadID ,event. messageID);
  	var key = `Có ${files.length} file có đuôi là: ${ext}`;
  }
  //all file
  else if (!args[0]) {
  if(files.length == 0) return api.sendMessage("Không có file hoặc folder nào", event.threadID ,event. messageID);
  var key = "Tất cả các file trong thư mục modules:";
  }
  //trong tên có ký tự.....
  else {
