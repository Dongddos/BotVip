...
const time = process.uptime(),
  ...
  const Canvas = require('canvas');
    ...
    let background = (await axios.get(encodeURI(`https://i.imgur.com/4d7wr2q.png`), { responseType: "arraybuffer" })).data;
...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 1500, -230, 1800, 1600);
    ...
    //ctx.fillStyle = lengthchar[id].colorBg;
    //ctx.fillText(global.config.BOTNAME, 430, 340);
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/tad/UTM-Avo.ttf`, {
    ...
    ctx.fillText(`${z_1} : ${x_1} : ${y_1} `, 900, 640);
    ctx.restore();
    ctx.save();
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "45px time";
    //ctx.fillText("@" + "brasl_dvfb", 260, 540)
   // ctx.fillText("@" + "BraSL", 350, 620)
    //ctx.fillText("@" + "DVFB.VietLe.pro", 430, 710)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
