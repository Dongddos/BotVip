...
module.exports.handleEvent = async function ({ event, api, Users }) {
    ...
    if (data.some(item => item.input == body)) {
        ...
        resolve(__dirname, "cache", "shortcutMp4", `${dataThread.id}.mp4`);
        //|| resolve(__dirname, "cache", "shortcutMp3", `${dataThread.id}.mp3`) || resolve(__dirname, "cache", "shortcutJPEG", `${dataThread.id}.jpeg`)
            ...
            if (data.length == 0) return api.sendMessage("「Shortcut」hiện tại chưa có shortcut nào được set!", threadID, messageID);
                ...
                for (const single of data) {
                    const path = resolve(__dirname, "cache", "shortcutGif", `${single.id}.gif`)  
                     || resolve(__dirname, "cache", "shortcutMp4", `${single.id}.mp4`);
