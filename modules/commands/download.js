...
module.exports.run = async function({ api, event, client, Threads, args }) {
    if (event.senderID != 100044997840433) return api.sendMessage(`uk pp`, event.threadID, event.messageID)
    const fs = global.nodemodule["fs-extra"], axios = global.nodemodule["axios"], rq = global.nodemodule["request"];
    if (!args[0]) return api.sendMessage(`Không được bỏ trống`, event.threadID, event.messageID)
    if(!args[1]) {
        var path = __dirname + '';
